...
//	public void playerConstr(EntityJoinWorldEvent event)
	{
		...
		if (!playerData.hasKey("ATTRIBUTES"))
		{
			...
			per.setFloat(Attributes.endurance.id, Attributes.endurance.transformToPerformance(sum[0]));
			per.setFloat(Attributes.agility.id, Attributes.agility.transformToPerformance(sum[1]));
			per.setFloat(Attributes.power.id, Attributes.power.transformToPerformance(sum[2]));
	...
	private static int[] generateLimitValue()
	{
		...
		return new int[] { i+101, j+101, k+101 };
	}
